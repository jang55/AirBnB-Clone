...
function UpdateReview({ setShowModal, review, pageType }) {
  ...
  const submitHandler = async (e) => {
    ...
    const submitReview = async () => {
      try {
        await dispatch(reviewActions.updateReviewThunk(formInfo, review, pageType));
        if (pageType === "detail") setNewReview(true);
        setShowModal(false);
      } catch (err) {
